    ...
    * lable addr_a -> print addr_a; */
    ...
    case LABEL:
        if (this->label.substr(0, 2) == ".L")
            ...
            fprintf(yyout, "addr_%s%d", this->label.c_str(), parent->getParent()->getParent()->getParent()->getNo());
    default:
        break;
    }
}
...
void MachineBlock::output()
{
    ...
    int num = ((FunctionType*)(parent->getSymPtr()->getType()))->getParamsSe().size();
    int count = 0;
    if (!inst_list.empty()) {
        fprintf(yyout, ".L%d:\n", this->no);
...
void MachineUnit::PrintGlobalDecl()
{
    ...
    for (long unsigned int i = 0; i < global_list.size(); i++) {
        ...
        if (se->getConst()) {
            ...
            else if (se->getType()->isInt())
                fprintf(yyout, "\t.word %d\n", (int)(se->getValue()));
            // 全局变量不能输出小数点
            else {
                float temp = se->getValue();
