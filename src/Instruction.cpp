...
ArrayInstruction::ArrayInstruction(Operand* dst, Operand* arr, Operand* idx, BasicBlock* insert_bb, bool isParam)
    : Instruction(ARRAY, insert_bb), isParam(isParam) {
    operands.push_back(dst);
...
void BinaryInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    * As to other instructions, such as MUL, CMP, you need to deal with this situation, too.*/
    ...
    case DIV:
        ...
        break;
    // 取模没有直接的运算，计算c=a%b，需要计算c=a-(a/b)*b
    case MOD:
        cur_inst = new BinaryMInstruction(cur_block, BinaryMInstruction::DIV, dst, src1, src2);
        tmp = new MachineOperand(*dst);
        src1 = new MachineOperand(*src1);
        src2 = new MachineOperand(*src2);
        cur_block->InsertInst(cur_inst);
        cur_inst = new BinaryMInstruction(cur_block, BinaryMInstruction::MUL, tmp, dst, src2);
        cur_block->InsertInst(cur_inst);
        dst = new MachineOperand(*tmp);
        cur_inst = new BinaryMInstruction(cur_block, BinaryMInstruction::SUB, dst, src1, tmp);
        break;
     case AND:
...
void CmpInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    cur_block->InsertInst(cur_inst);
    if (opcode != CmpInstruction::EQ && opcode != CmpInstruction::NEQ) {
        auto dst = genMachineOperand(operands[0]);
        ...
        cur_block->InsertInst(cur_inst);
        int mopcode;
        if (opcode == CmpInstruction::GEQ || opcode == CmpInstruction::GRA)
            mopcode = opcode + 1;
        else
            mopcode = opcode - 1;
        cur_inst = new MovMInstruction(cur_block, MovMInstruction::MOV, dst, falseOperand, mopcode);
        cur_block->InsertInst(cur_inst);
    }
}
...
void CondBrInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    //std::string str = ".L" + true_branch->getNo();
    MachineOperand* dst = new MachineOperand(s.str());
    int op= cur_block->getCmpOp();
    int mtype=MachineInstruction::NONE;
    switch (op) {
        case 1:
            mtype = MachineInstruction::GE;
            break;
        case 4:
            mtype = MachineInstruction::GT;
            break;
        case 2:
            mtype = MachineInstruction::LE;
            break;
        case 5:
            mtype = MachineInstruction::LT;
            break;
        case 3:
            mtype = MachineInstruction::NE;
            break;
        case 0:
            mtype = MachineInstruction::EQ;
            break; 
    }
    // 条件跳转
    auto cur_inst = new BranchMInstruction(cur_block, BranchMInstruction::B, dst, mtype);
    cur_block->InsertInst(cur_inst);
    //str = ".L" + false_branch->getNo();
    s.str("");
