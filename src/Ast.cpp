        ...
        if (arrIdx) {
            ...
            while (true) {
                ...
                if (empty)
                    empty = false;
                // 维度要全部换成整数   
                if (elemType == TypeSystem::intType || elemType == TypeSystem::constIntType
                    || elemType == TypeSystem::floatType || elemType == TypeSystem::constFloatType)
                    break;
...
void BinaryExpr::typeCheck()
{
    expr1 -> typeCheck();
    expr2 -> typeCheck();
    // 关于expr1和expr2需要类型一致的检查写到构造函数里了，因为parser.y就需要
}
...
void ReturnStmt::typeCheck()
{
    ...
    if(retValue) {
        retValue->typeCheck();
        int kind1 = retValue->getType()->getKind();
        int kind2 = retType->getKind();
        // int和array类型的kind值差5，int和float类型的kind差6
        if(abs(kind1-kind2)==5 || abs(kind1-kind2)==6
            || ((kind1==5 || kind1==6) && (kind2==5 || kind2==6))) ;
        else if(kind1!=kind2) {
...
void AssignStmt::typeCheck()
{
    ...
    if (!expr->getType()->isInt() && !expr->getType()->isArray() && !expr->getType()->isFloat()) {
...
void CallFunc::typeCheck() {
    ...
    if (symbolEntry) {
        ...
        for (auto it = params.begin(); it != params.end(); it++) {
            ...
            temp->typeCheck();
            int kind1 = (*it)->getKind();
            int kind2 = temp->getType()->getKind();
            // int和array类型的kind值差5，int和float类型的kind差6
            if(abs(kind1-kind2)==5 || abs(kind1-kind2)==6
                || ((kind1==5 || kind1==6) && (kind2==5 || kind2==6))) ;
            else if(kind1!=kind2) {
        ...
        if (temp != nullptr && !((IdentifierSymbolEntry*)symbolEntry)->isSysy()) {
...
BinaryExpr::BinaryExpr(SymbolEntry *se, int op, ExprNode*expr1, ExprNode*expr2) 
    ...
    // 两个表达式为数组、整型、浮点型都可以，结果表达式的类型为更精确的那个
    if ((type1->isArray()||type1->isInt()||type1->isFloat())&&(type2->isArray()||type2->isInt()||type2->isFloat())) {
